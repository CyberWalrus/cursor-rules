...
- **Plan Mode** — анализ задачи, классификация типа (8 activity types), создание плана с подтверждением
...
- Классифицирует как `Code Development`
- Создаёт план: декомпозиция на 8 задач (middleware, tokens, tests, docs)
- Определяет зависимости и порядок выполнения
- Запрашивает подтверждение перед стартом
